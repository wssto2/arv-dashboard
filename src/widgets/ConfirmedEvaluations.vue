...
type ConfirmedEvaluations = {
	less_than_30_days: number;
	less_than_60_days: number;
	more_than_60_days: number;
}

const isFetching = ref(false);
const data = ref<ConfirmedEvaluations>({
	less_than_30_days: 0,
	less_than_60_days: 0,
	more_than_60_days: 0
});
...
const fetch = async () => {
	...
	try {
		...
		const endpoint = joinPath(window?.DASHBOARD_STATE?.api_base_url || '', '/widgets/confirmed_evaluations');
...
const formattedData = computed(() => {
	return {
		...
		datasets: [
			{
				data: [
					data.value.less_than_30_days || 0,
					data.value.less_than_60_days || 0,
					data.value.more_than_60_days || 0
				],
		...
		<div class="panel-heading">
			<div style="display: flex; align-items: center;">
				<h6 class="panel-title" style="flex: 1;">
					<span class="text-default">{{ t('confirmed_evaluations.title') }}</span>
				</h6>
